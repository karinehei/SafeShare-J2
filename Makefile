...
# --allow-fs belongs on j2, before the program file. Bare `j2 FILE args` forwards argv.
RUN := $(J2) --allow-fs $(MAIN)
...
help:
	...
	@echo "  make build              j2 build $(MAIN) -o $(BIN)  (--help only; scan still needs j2)"
	@echo "  make scan               j2 --allow-fs src/main.j2 scan SCAN --ai-share (default $(SCAN))"
	...
	@echo "  make benchmark-native   time CORPUS after j2 build (still via j2 --allow-fs)"
	@echo "  make smoke              native compile, then interpreter smoke of demo-project"
	@echo "  make self-scan          native compile, then interpreter scan of this repo"
...
smoke: $(BIN)
	...
	test -d demo-project; \
	$(RUN) scan demo-project --json "$$json1" | tee "$$log"; \
	$(RUN) scan demo-project --json "$$json2" > /dev/null; \
	cmp -s "$$json1" "$$json2"; \
...
self-scan: $(BIN)
	...
	$(RUN) scan . --ai-share --json "$$json" | tee "$$tmp/self-scan.txt"; \
